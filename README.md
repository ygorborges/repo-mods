...
- **[UsableValuables](UsableValuables/)** - hold a valuable that does something (flashlight, boombox, candle, ice saw, blender, jackhammer, scream doll, fan, radio, gramophone, television, toy monkey, grandfather clock, flamethrower, fire extinguisher, star wand, wizard staff, camera, levitation potion) and press Interact (E) to use it, and makes the banana bow (lights a fuse and blows up), the handface (laughs and shakes when held; turns to face and is flung, once, at players who keep moving near it when not), the horse (whinnies the first time it is picked up) and the piano (plays a Liszt rhapsody when held by anything but its keys) act up by themselves, and adds two valuables of its own: a dish sponge that blows bubbles and drips when moved, and a bar of soap that hops around on its own like a rubber duck and knocks enemies down when they touch it (needs [REPOLib](https://thunderstore.io/c/repo/p/Zehs/REPOLib/), and every player in the room to have the mod). On [Thunderstore](https://thunderstore.io/c/repo/p/vibez/UsableValuables/) (the soap is not in the published release yet).
- **[MoonControl](MoonControl/)** - the moon no longer rises on its own; it stays off until you grab the moon button at a fixed spot in the truck (the same way you hold the extraction point's own button), and only the host can pick a moon your run has actually unlocked - or "Random", which keeps the pick a surprise until the next level starts. Doing so adds 10% to every valuable's worth per level applied. Needs [REPOLib](https://thunderstore.io/c/repo/p/Zehs/REPOLib/). Tested in the game. On [Thunderstore](https://thunderstore.io/c/repo/p/vibez/MoonControl/).
- **[PaceControl](PaceControl/)** - makes some of a level's extraction points optional: say how many it actually needs and the rest are still built, still unlocked and still worth the same, just not required. The level's quota comes down by the same proportion, so every extraction still asks for exactly what it always did and a late-game level can be half the evening. The end-of-level state (the scare, the lights going out, everything coming for you) waits until the truck is pulling out, so the optional ones are playable. Every player in the room needs the mod; the host's settings are the ones used. Not yet tested in the game, not published.
- **[DevTools](DevTools/)** - my testing shortcuts (not published, no Thunderstore package). **F9**: adds $100K and, when you are inside a level, wins it and goes to the shop. **F10**: spawns a random valuable that UsableValuables works on in front of you. **F8**: spawns the dish sponge. **F7**: spawns the soap. **F6**: toggles a coordinate probe (world XYZ under the crosshair, for placing things); **F5** logs its current reading. Host/singleplayer only; keys and amount are in `BepInEx/config/vibez.DevTools.cfg`.
